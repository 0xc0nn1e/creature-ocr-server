...
`/v1` は一文字も変わっていない。同じ画像を送れば同じ行が同じ `settings` で返る。
v2 は**別の紙のための別の口**で、`/v1` を置き換えるものではない。

### なぜ別の版なのか

小学校、年、組は印刷された見出し帯にあり、**名前と自宅住所と同じ行に並んでいる**。
帯ごと切れば個人情報が付いてくるので、アプリ側が帯の左半分だけを切り出し、表の
下に貼った一枚の合成画像を送る。v2 が受け取るのはその合成画像。

三つの値は**一枚に一つ**で、行に一つではない。だから `OCR_FIELDS` には入れず、
`sheet_definition` にも入れない ― 入れれば `checks_fingerprint` が動き、v1 の
クライアントが持っているキャッシュが**一枚残らず無効になる**。実際に動いていない
ことは下の確認方法で見られる。

### 合成画像の寸法は契約

    表  3367 × 1442 を (0, 0) に原寸で貼る（v1 の切り抜きそのもの）
    全体 3507 × 1620

`config.V2_CANVAS_PIXELS` がこれ。**別の寸法で作られた合成画像は、値を間違った
セルから読む**。しかも二つの枠は原点付近では重なっているので、上の行は正しく読め、
下の行だけが空になる ― いちばん気づきにくい壊れ方をする。サーバ側にそれを見抜く
手段は無い。

v2 の座標は転記ではなく導出。表が原寸で貼られている以上、v2 の分数は v1 の分数に
軸ごとの比を掛けたものであり、`config.py` はそう書いてある。v1 の格子を測り直せば
v2 が付いてくるし、どちらも打ち間違えようがない。

### 三つのエンドポイント

| | |
|---|---|
| `POST /v2/ocr` | `/v1/ocr` と同じ multipart。応答は v1 の全項目 + `school` / `grade` / `school_class` |
| `GET /v2/engines` | `/v1/engines` と同じ形。`settings` と `cache_name` だけが違う |
| `GET /v2/sheet` | `/v1/sheet` の内容 + `header_fingerprint` と `header` |

    curl -sS http://127.0.0.1:8000/v2/ocr \
      -H "X-API-Key: $SERVER_API_KEY" \
      -F "image=@output/生き物/page01_v2.png" \
      -F "engine=gemini" -F "model=gemini-3.7-flash"

応答は `POST /v1/ocr` のものに三項目が足されただけ:
...
`school_class` であって `class` ではない。Python の予約語なので、クライアントが
読めてサーバが自分のコードで名指せない項目を作らない。

年と組は数字だけ。数字以外が来たら**近い数字に直さず空文字列にする**。5.2-2 の
規則は見出し帯にもそのまま及ぶ。

### キャッシュは版ごとに分かれる

    v1  gemini-3.7-flash temperature=0.0 top_p=0.1 grid=d103bef3 prompt=7b48ed75 checks=79ebbe0c
    v2  gemini-3.7-flash sheet=v2 temperature=0.0 top_p=0.1 grid=de202e9e prompt=0cc22836 checks=79ebbe0c+f9c84317

`cache_name` も `gemini-3.7-flash` と `gemini-3.7-flash-v2` に分かれる。同じ紙の
二つの読み取りは**二つの読み取り**であって、一つの名前に置けば片方がもう片方を
上書きする。engine は `(engine, model, sheet)` ごとに一つ組み立てられる。

### 座標エンジンだけが気にする一点

年 と 組 は**一つの枠を分け合っていて、間に印刷された罫線が無い**。`0.2196` という
境界は紙の上の線ではなく、年 という印字の右端。gemini は影響を受けない ― 何を
返すか言葉で指示されている ― が、`documentai` と `nemotron` はこの分割をそのまま
読む。v2 の座標で唯一、測定ではなく判断なのはここだけ。
...
二つの読み取りを一つの名前に置けば、片方がもう片方を上書きする。
...
- **v2 の埋まった見出し帯での検証。** 合成画像の幾何は実測で合わせてあり、表の
  8 行は v1 と 1 セル残らず一致することを確認済み。だが手元の合成サンプルは
  見出し帯が**空欄**なので、小学校 / 年 / 組 が実際に読めるかはまだ通していない。
  **文字の入った合成画像が一枚要る。**
